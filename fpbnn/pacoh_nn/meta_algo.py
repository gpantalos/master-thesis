...
class MetaLearner:
    ...
    def _process_meta_train_data(self, meta_train_data, meta_batch_size, batch_size, n_batched_models_train):
        ...
        self.meta_train_sampler = modules.MetaDatasetSampler(
            meta_train_data, batch_size,
            meta_batch_size=meta_batch_size,
            n_batched_models=n_batched_models_train,
            tiled=True
        )
    ...
    def _meta_test_training_loop(self, test_tasks, eval_models, eval_models_step,
        ...
        for i in range(num_iter):
            ...
            if i > 0 and i % log_period == 0:
                ...
                message = '\tMeta-Test - Iter %d/%d - Time %.2f sec' % (i, num_iter, time() - t)
                message += ' - Train-Log-Prob: %.3f' % avg_log_prob
                print(message)
                t = time()

            # meta-test evaluation
            if i > 0 and i % eval_period == 0:
                t = time()
                eval_metrics_mean, eval_metrics_std, _ = self._meta_test_models_eval(test_tasks, eval_models)
                message = ""
                for key in eval_metrics_mean:
                    message += '- Val-%s: %.3f +- %.3f' % (key, eval_metrics_mean[key][0], eval_metrics_std[key][0])

                print('\n\t Iter %d/%d - Time %.2f sec %s' % (i, num_iter, time() - t, message))
