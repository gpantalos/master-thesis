...
class PACOH_NN_Regression(MetaLearner):
    ...
    def __init__(self, meta_train_data, lr=2e-3, hidden_layer_sizes=(32, 32, 32, 32), activation='relu',
                 ...
                 hyper_prior_likelihood_log_var_log_var_mean=-4., bandwidth=10.0, random_seed=None):
        ...
        self.hyper_prior_module = modules.GaussianHyperPrior(
            self.prior_module,
        ...
        self.hyper_posterior_particles = tf.Variable(tf.expand_dims(
            self.prior_module.get_variables_stacked_per_model(), axis=0))
    ...
    def meta_fit(self, meta_val_data=None, log_period=500, eval_period=1000, plot_prior_during_training=False,
        ...
        plot_prior_during_training = plot_prior_during_training and (self.input_dim == self.output_dim == 1)
        log_prob = 0

        print("Start meta-training -------------------- ")
        t = time.time()
        for i in range(self.num_iter_meta_train):
            ...
            message = ''
            # print training loss
            if plot_prior_during_training and i % plot_period == 0:
                self.plot_prior(plot_pred_lines=True, plot_data=True, max_task_to_plot=10,
                                title='prior samples @ iter %i' % i, show=True)

            if i % log_period == 0 or i % eval_period == 0:
                avg_log_prob = tf.reduce_mean(log_prob).numpy()
                message += '\nIter %d/%d - Time %.2f sec' % (i, self.num_iter_meta_train, time.time() - t)
                message += ' - Train-Loss: %.5f' % avg_log_prob

                # run validation and print results
                if meta_val_data is not None and i % eval_period == 0 and i > 0:
    ...
    def meta_eval_datasets(self, meta_valid_data, max_tasks_parallel=5):
        ...
        print(f"\tStart meta-test posterior inference in {len(eval_tasks_batches):d} batches ------------------")

        eval_metrics_dict_per_task = []
        for eval_batch_id, task_batch in enumerate(eval_tasks_batches):
            print(f"\tMeta-Test batch #{eval_batch_id + 1:d} consisting of {len(task_batch):d} tasks----")
        ...
        return self._aggregate_eval_metrics_across_tasks(eval_metrics_dict_per_task)
    ...
    def _pacoh_log_prob_and_grad(self, meta_batch_x, meta_batch_y, n_train_samples):
        with tf.GradientTape() as tape:
            ...
            log_likelihood = self._estimate_mll(self.hyper_posterior_particles, meta_batch_x, meta_batch_y,
                                                n_train_samples)
            log_prior_prob = self.hyper_prior_module.log_prob_vectorized(
                self.hyper_posterior_particles,
    ...
    def _estimate_mll(self, prior_samples, meta_batch_x, meta_batch_y, n_train_samples):
        ...
        mll_sum = tf.reduce_sum(mll, axis=0) * (
                self.meta_train_sampler.n_tasks / self.meta_train_sampler.meta_batch_size)
    ...
    def _compute_likelihood_across_tasks(self, params, meta_batch_x, meta_batch_y):
        """ compute the average likelihood, i.e. the mean of the likelihood for the points in the batch (x,y)
            ...
            Returns:
                log_likelihood_across_tasks with shape (meta_batch_size, n_hyper_posterior_samples, n_prior_samples)
        """
