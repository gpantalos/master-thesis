...
class GaussianLikelihood(tf.Module):
    def __init__(self, output_dim, n_batched_models, std=0.2, trainable=True, name='gaussian_likelihood'):
        ...
        if trainable:
            self.log_std = tf.Variable(tf.cast(tf.ones(n_batched_models, output_dim), tf.float32) * tf.math.log(std),
                                       trainable=True)
        else:
            self.log_std = tf.constant(tf.cast(tf.ones(n_batched_models, output_dim), tf.float32) * tf.math.log(std))
    ...
    def calculate_eval_metrics(self, pred_dist, y_true):
        eval_results = dict(
            nll=-tf.reduce_mean(pred_dist.log_prob(y_true)).numpy(),
            rmse=self.rmse(pred_dist.mean(), y_true).numpy()
        )

        if self.output_dim == 1:
            eval_results.update(dict(cal_err=self.calib_error(pred_dist, y_true).numpy()))
