...
class TFModuleBatched(tf.keras.Model):
    ...
    def get_variables_vectorized(self):
        return tf.concat([tf.reshape(v, (1, -1)) for v in self.weights], axis=1)
    ...
    def variable_sizes(self):
        if self._variable_sizes is None:
            self._variable_sizes = [tf.size(v) for v in self.weights]
    ...
    def call_parametrized(self, x, variables_vectorized):
        ...
        with tape:
            tape.watch([x] + list(self.trainable_weights))
            y = self.call(x, )

        def grad_fn(dy, variables):
            with tape:
                tampered_y = y * dy
            grads_x_w = tape.gradient(tampered_y, [x] + list(self.trainable_weights))
