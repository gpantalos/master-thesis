...
class MetaDatasetSampler:
    ...
    def __init__(self, data, per_task_batch_size, meta_batch_size=-1, n_batched_models=None, tiled=False,
        ...
        if per_task_batch_size == -1:
            ...
            assert len(
                set(n_samples_per_task)) == 1, "n_samples differ across tasks --> per_task_batch_size must be set > 0"
        ...
        task_id_sampler = tf.data.Dataset.from_tensor_slices(tasks_ids).shuffle(
            self.n_tasks,
...
class DatasetSampler:
    def __init__(self, train_data, val_data, batch_size, n_batched_models=None, tiled=False, flatten_data=False,
        ...
        if batch_size > self.n_train_samples:
            print(f"NOTE: The requested batch size `{batch_size}` "
                  f"is bigger the number of training samples `{self.n_train_samples}`")
        ...
        if val_data is not None:
            ...
            x_min_val, x_max_val = tf.math.floor(tf.reduce_min(self.val_data[0])), tf.math.ceil(
                tf.reduce_max(self.val_data[0]))
...
def _split_into_batches(list_, max_batch_size):
    import math
    n_elements = len(list_)
    if max_batch_size == -1:
    ...
    for i in range(n_batches):
        ...
        batches.append(list_[idx:idx + batch_size])
        idx += batch_size
