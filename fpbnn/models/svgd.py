...
class SVGD(BayesianRegressionModel):
    ...
    def __call__(self, x_batch, y_batch):
        with tf.GradientTape() as tape:
            ...
            log_p = self.prior.log_prob(self.particles)
            log_q = likelihood + log_p * self.coeff_prior

        # compute gradient of log-log_q wrt particles
        grad_q = tape.gradient(log_q, self.particles)

        # compute kernel matrix and grads
        particles_copy = tf.identity(self.particles)
        with tf.GradientTape() as tape:
            tape.watch(self.particles)
            k = self.kernel.matrix(self.particles, particles_copy)
        grad_k = tape.gradient(k, self.particles)
        svgd_grads = k @ grad_q + grad_k / self.n_particles
        self.optimizer.apply_gradients([(-svgd_grads, self.particles)])
