...
from .batched_module import TFModuleBatched
...
class BatchedFullyConnectedNN(TFModuleBatched):
    ...
    def __init__(self, n_batched_models, output_size, hidden_layer_sizes=(32, 32), activation='relu', name='batched_nn'):
    ...
    def call(self, inputs, batched_input=False):
        if batched_input:
            # inputs is a tensor with dimensions (n_batched_models x batch_size x input_shape)
            ...
            # inputs is a tensor with dimensions (batch_size x input_shape)
            tf.assert_equal(len(inputs.shape), 2)
            outputs = tf.stack([self.models[i](inputs) for i in range(self.n_batched_models)])
        # outputs is a tensor with dimensions (n_batched_models x batch_size x output_shape)
        return outputs